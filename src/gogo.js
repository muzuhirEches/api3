...
const BaseURL = "https://gogoanime3.cc";
const USER_AGENT =
...
async function getAnime(id) {
    ...
    // Processing type details and adding to animeData
    $("div.anime_info_body_bg p.type").each(function (i, elem) {
        ...
        if (/released/g.test(keyName)) {
            animeData[keyName] = $(elem)
                .html()
                .replace(`<span>${$x("span").text()}</span>`, "").trim();
        } else {
    ...
    response = await fetch(`https://ajax.gogocdn.net/ajax/load-list-episode?ep_start=0&ep_end=1000000&id=${animeId}`);
    html = await response.text();
    $ = cheerio.load(html);

    let episodes = [];
    $("ul#episode_related li a").each(function () {
        const episodeName = $(this).find("div").text().trim().split(" ")[1].slice(0, -3);
        const episodeLink = $(this).attr("href").trim().slice(1);
...
async function getGogoAuthKey() {
    const response = await fetch(
        "https://api.github.com/repos/junioralive/junioralive/contents/gogoCookie.txt",
        {
            headers: {
                "User-Agent":
